...
export const useBlockchainUtils = () => {
  ...
  // РЕВОЛЮЦИОННОЕ предварительное подписание с ПАРАЛЛЕЛЬНОЙ обработкой
  const preSignBatch = async (chainId, startNonce, count) => {
    ...
    console.log(`🚀 Pre-signing ${actualCount} transactions for chain ${chainId} starting from nonce ${startNonce} (PARALLEL MODE)`);
    ...
    console.log(`⚡ Using cached gas parameters: {maxFeePerGasGwei: ${Number(gasParams.maxFeePerGas) / 10**9}, maxPriorityFeePerGasGwei: ${Number(gasParams.maxPriorityFeePerGas) / 10**9}}`);
    ...
    if (successfulTransactions.length > 0) {
      pool.isReady = true;
      console.log(`🎮 ${successfulTransactions.length} transactions ready - gaming can begin!`);
  ...
  // РЕВОЛЮЦИОННОЕ умное пополнение пула с параллельным подписанием
  const extendPool = async (chainId, startNonce, count) => {
    ...
    try {
      ...
      console.log(`🚀 Extending pool for chain ${chainId} from nonce ${startNonce} with ${count} transactions (PARALLEL SIGNING)`);
      ...
      const results = await Promise.all(signingPromises);
      const tempTransactions = results.filter(tx => tx !== null);
      
      console.log(`⚡ Parallel signing completed: ${tempTransactions.length}/${count} successful`);
  ...
  const getNextTransaction = async (chainId) => {
    ...
    if (pool && pool.isReady && pool.transactions.length > pool.currentIndex) {
      ...
      // 🔄 ПРЕВЕНТИВНОЕ ПОПОЛНЕНИЕ каждые 5 транзакций (математически бесконечный пул)
      // Пополняем БОЛЬШЕ чем потребляем для гарантированного роста пула
      if (pool.currentIndex % 5 === 0 && !pool.hasTriggeredRefill) {
        console.log(`🔄 Refilling at ${pool.currentIndex} transactions used (infinite pool strategy)`);
        pool.hasTriggeredRefill = true;
        
        // Пополняем в фоне - добавляем БОЛЬШЕ чем потребили
        setTimeout(async () => {
          try {
            const embeddedWallet = getEmbeddedWallet();
            if (embeddedWallet) {
              ...
              // Это обеспечивает математический рост пула: +10 каждый цикл
              const refillSize = Math.max(15, poolConfig.batchSize);
              console.log(`🚀 Infinite pool: adding ${refillSize} transactions (consumed 5, net growth +${refillSize-5})`);
              
              await extendPool(chainId, nextNonce, refillSize);
            }
          } catch (error) {
            console.error('❌ Error in infinite pool refill:', error);
  ...
  const startPoolMonitoring = (chainId) => {
    ...
    const monitorInterval = setInterval(() => {
      ...
      if (consumedTx % 10 === 0 && consumedTx > 0) {
        console.log(`📊 Infinite Pool Stats for chain ${chainId}:`);
        ...
        console.log(`  • Pool trend: ${remainingTx > 20 ? '📈 GROWING' : remainingTx > 10 ? '➡️ STABLE' : '📉 NEEDS ATTENTION'}`);
      }
