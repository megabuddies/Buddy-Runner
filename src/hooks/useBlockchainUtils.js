...
const NETWORK_CONFIGS = {
  6342: { // MegaETH Testnet
    ...
    chainId: 6342,
    sendMethod: 'eth_sendRawTransaction', // Отключен realtime: используем только pre-signed пул
    connectionTimeouts: {
        ...
        setTimeout(async () => {
          try {
            ...
            if (embeddedWallet) {
              const manager = getNonceManager(chainId, embeddedWallet.address);
              const nextNonce = manager.pendingNonce;
              const stableRefill = Math.max(8, Math.floor((poolConfig.refillBatchSize || 10)));
              await extendPool(chainId, nextNonce, stableRefill);
            }
          } catch (error) {
