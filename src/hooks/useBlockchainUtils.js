...
export const useBlockchainUtils = () => {
  ...
  const ENHANCED_POOL_CONFIG = {
    6342: { // MegaETH - МАКСИМАЛЬНАЯ ПРОИЗВОДИТЕЛЬНОСТЬ
      ...
      batchSize: 500, // ЗНАЧИТЕЛЬНО БОЛЬШИЙ размер пакета для опережающего пополнения
      maxRetries: 3,
    ...
    31337: { // Foundry
      poolSize: 10000, // УВЕЛИЧЕН до 10000 для длинных игровых сессий
      refillAt: 0.2, // Более раннее пополнение
      batchSize: 300, // Больший размер пакета
      maxRetries: 3,
      retryDelay: 150,
      burstMode: true,
    ...
    50311: { // Somnia
      poolSize: 10000, // УВЕЛИЧЕН до 10000 для длинных игровых сессий
      refillAt: 0.2, // Более раннее пополнение
      batchSize: 250, // Больший размер пакета
      maxRetries: 3,
      retryDelay: 300,
      burstMode: true,
    ...
    1313161556: { // RISE
      poolSize: 10000, // Увеличен до 10000 для pre-signed only
      refillAt: 0.4,
      batchSize: 120,
      maxRetries: 2,
      retryDelay: 400,
  ...
  const preSignBatch = async (chainId, startNonce, count) => {
    ...
    const PARALLEL_BATCH_SIZE = 100; // Подписываем по 100 транзакций параллельно для больших пулов
  ...
  const getNextTransaction = async (chainId) => {
        ...
        setTimeout(async () => {
          try {
            ...
            if (embeddedWallet) {
              ...
              const baseRefillSize = Math.max(350, poolConfig.batchSize * 2);
        ...
        setTimeout(async () => {
          try {
            ...
            if (embeddedWallet) {
              const manager = getNonceManager(chainId, embeddedWallet.address);
              const nextNonce = manager.pendingNonce;
              const emergencyRefillSize = Math.max(500, poolConfig.batchSize * 2.5);
    ...
    try {
      ...
      if (embeddedWallet) {
        ...
        const emergencyBatchSize = Math.min(50, poolConfig.batchSize);
        const nextNonce = manager.pendingNonce;
