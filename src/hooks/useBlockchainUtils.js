...
export const useBlockchainUtils = () => {
  ...
  const ENHANCED_POOL_CONFIG = {
    6342: { // MegaETH - МАКСИМАЛЬНАЯ ПРОИЗВОДИТЕЛЬНОСТЬ
      poolSize: 10000, // УВЕЛИЧЕН до 10000 для решения проблемы после 150 транзакций
      refillAt: 0.15, // БОЛЕЕ раннее пополнение при 15% использования
      batchSize: 2500, // ОГРОМНЫЙ размер пакета для опережающего пополнения
      maxRetries: 3,
    ...
    31337: { // Foundry
      poolSize: 6000, // УВЕЛИЧЕН до 6000 для длинных игровых сессий
      refillAt: 0.2, // Более раннее пополнение
      batchSize: 1500, // Огромный размер пакета
      maxRetries: 3,
      retryDelay: 150,
      burstMode: true,
    ...
    50311: { // Somnia
      poolSize: 5000, // УВЕЛИЧЕН до 5000 для длинных игровых сессий
      refillAt: 0.2, // Более раннее пополнение
      batchSize: 1250, // Огромный размер пакета
      maxRetries: 3,
      retryDelay: 300,
      burstMode: true,
    ...
    1313161556: { // RISE
      poolSize: 2500, // УВЕЛИЧЕН до 2500 для pre-signed only
      refillAt: 0.4,
      batchSize: 600,
      maxRetries: 2,
      retryDelay: 400,
    ...
    default: {
      poolSize: 3000, // УВЕЛИЧЕН до 3000 для pre-signed only
  ...
  const preSignBatch = async (chainId, startNonce, count) => {
    ...
    const PARALLEL_BATCH_SIZE = 50; // Подписываем по 50 транзакций параллельно для огромных пулов
  ...
  const extendPool = async (chainId, startNonce, count) => {
    ...
    try {
      ...
      if (pool && tempTransactions.length > 0) {
        ...
        }, 5000); // Увеличенная задержка для предотвращения частых пополнений при больших пулах
      }
    } catch (error) {
  ...
  const getNextTransaction = async (chainId) => {
    ...
    if (pool && pool.isReady && pool.transactions.length > pool.currentIndex) {
      ...
      // Пополняем каждые 500 транзакций большими батчами для минимизации блокировок
        ...
        setTimeout(async () => {
          try {
            ...
            if (embeddedWallet) {
              ...
              const baseRefillSize = Math.max(1750, poolConfig.batchSize * 2);
        ...
        setTimeout(async () => {
          try {
            ...
            if (embeddedWallet) {
              const manager = getNonceManager(chainId, embeddedWallet.address);
              const nextNonce = manager.pendingNonce;
              const emergencyRefillSize = Math.max(2500, poolConfig.batchSize * 2.5);
